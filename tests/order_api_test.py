...
class OrderApiTest(unittest.TestCase):
    ...
    def callback_test(self):
        req = requests.get('http://op.yikao666.cn/JDTrainOpen/CallBackForTN?id=%d&success=%s' % (13915, 'false'))
        resp = req.json()
    ...
    def post_order_test(self):
        req = requests.post('http://localhost:8000/api/mobilepay/order', data=
            ...
            '"partner_order_id":null,"status_id":null,"status_name":null,"account_info":null,"coupon_id":null,"get_time":null,"JdUserInfo":null}',headers={'Content-Type':'application/json'})
        self.assertIsNotNone(req.text)

    def set_order_status_test(self):
        req = requests.put(u'http://localhost:8000/api/mobilepay/order/status/10001/下单失败')
