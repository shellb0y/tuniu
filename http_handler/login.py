...
def begin_session(partner):
    ...
    c = {"v": "8.1.6", "ct": 20, "dt": 1, "ov": 1, "p": partner, "cc": 2500}
    url = 'https://m.tuniu.com/api/user/auth1/beginSession?c=%s' % json.dumps(c)
...
# 0
def login(username, password, partner):
    req = requests.get('https://m.tuniu.com/api/user/auth/login', {
        'd': {"captcha": "", "deviceId": "31f432734f0fb42f2bc55352abc1:28",
              "sessionId": "bb3fdf37ef796386b446181279323a64", "loginId": "15728532201",
              "password": "f88be3495c9c29561a17c8755b073ebf", "isDynamic": 0},
        'c': {"v": "8.1.6", "ct": 20, "dt": 1, "ov": 1, "p": partner, "cc": 2500}})
